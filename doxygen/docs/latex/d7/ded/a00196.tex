...
\begin{DoxyItemize}
\item Initialize the stack to be empty. {\ttfamily init\+\_\+stack()}
\item Determine whether the stack is empty or not {\ttfamily is\+\_\+empty()}
\item Check whether the stack is full or not {\ttfamily is\+\_\+full()}
\item If the stack is not full, add or insert a new node at the top of the stack. This operation is termed as Push Operation {\ttfamily push()}
\item If the stack is not empty, then retrieve the node at its top {\ttfamily peek()}
\item If the stack is not empty, the delete the node at its top. This operation is called as Pop operation. {\ttfamily pop()} 
\end{DoxyItemize}